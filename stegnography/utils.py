...
def waveAudioEncrypt(songs,messages,audioId):
    ...
    bits = list(map(int, ''.join([bin(ord(i)).lstrip('0b').rjust(8,'0') for i in string]))) # hereak bit lai ek ek bit ma tukrauney

    # Replace LSB of each byte of the audio data by one bit from the text bit array
    for i, bit in enumerate(bits):
        frame_bytes[i] = (frame_bytes[i] & 254) | bit # mian bit ko last ma append garenye code
    
    frame_modified = bytes(frame_bytes)
    
    
    with wave.open("{}.wav".format(audioId),'wb') as fd:
...
def waveAudioDecrypt(audio):
    ...
    decoded = string.split("###")[0] # if more than 3 hash are found then split the data 
